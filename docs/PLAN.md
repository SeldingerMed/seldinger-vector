...
| V-10 | **Minimum rendered size of burned-in identifiers across in-scope capture systems** | Not measured. The overlay detector's recall is bounded by its grid resolution, and `SAFE_OVERLAY_MIN_PX` (8px) is a conservative ceiling, not a finding. **Because V-10 is open, the platform does not attest by default**: `redact` refuses unless a deployment records its own measurement in `overlay_bound_validated_against`. Discharging V-10 is therefore a precondition of attesting at all, per deployment. | ML lead + clinical lead | Phase 0 — measure across the capture hardware actually in scope |
